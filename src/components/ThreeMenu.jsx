...
function FloatingCard({ basePos, label, onClick, isActive }) {
  const mesh = useRef();
...
export default function ThreeMenu({ onOpenPage, onClosePage, activePage }) {
  ...
  // Učitavanje tekstura Meseca i Merkura
  ...
  return (
    <div className="canvas-wrap" style={{ width: "100vw", height: "100vh", backgroundColor: "#0a0a0a" }}>
      <Canvas camera={{ position: [0, 0, 8], fov: 50 }}>
        ...
        <group ref={sceneGroup}>
          ...
          <mesh position={[0, -10, 0]} onClick={() => { if (activePage) handleClose(); }}>
            <planeGeometry args={[200, 200]} />
            <meshBasicMaterial opacity={0} transparent />
          </mesh>
        </group>

        {/* Efekti svetlosti i dubine */}
        <EffectComposer>
